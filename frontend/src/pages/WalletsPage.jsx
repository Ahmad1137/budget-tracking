...
function WalletsPage() {
  ...
  const fetchWallets = async () => {
    ...
    try {
      ...
      walletsRes.data.forEach((wallet) => {
        const walletTransactions = transactionsRes.data.filter(
          (t) => t.walletId === wallet._id
        );
        const totalIncome = walletTransactions
          .filter((t) => t.type === "income")
          .reduce((sum, t) => sum + t.amount, 0);
        const totalExpenses = walletTransactions
  ...
  const handleJoin = async (e) => {
    ...
    try {
      ...
      toast.success(
        "Wallet Joined",
        `Successfully joined wallet: ${res.data.name}`
      );
    } catch (err) {
  ...
  const deleteWallet = async (walletId) => {
    if (window.confirm("Are you sure you want to delete this wallet?")) {
      try {
        ...
        toast.success(
          "Wallet Deleted",
          "Wallet has been successfully deleted."
        );
      } catch (err) {
  ...
  const getTotalStats = () => {
    const totalBalance = Object.values(walletStats).reduce(
      (sum, stats) => sum + stats.balance,
      0
    );
    const totalTransactions = Object.values(walletStats).reduce(
  ...
  return (
    <div className="container mx-auto mt-14 px-4">
      ...
      {/* Empty Wallets Alert */}
      {wallets.length > 0 &&
        Object.values(walletStats).every((stat) => stat.balance <= 0) && (
          <div
            className={`mb-6 p-4 ${
              isDark
          ...
          >
            <div className="flex items-start space-x-3">
              ...
              <div className="flex-1">
                ...
                >
                  Your wallets are empty! Add some income first before creating
      ...
      {showJoinForm && (
        ...
        >
          ...
          <p className={`${isDark ? "text-gray-500" : "text-gray-900"} mb-3`}>
            Copy wallet id below to join the wallet
          </p>
      ...
      {filteredWallets.length === 0 ? (
        <div className="text-center py-12">
          ...
          <p className={`${isDark ? "text-gray-400" : "text-gray-500"}`}>
            Create your first wallet or join an existing one
          </p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredWallets.map((wallet) => {
            const stats = walletStats[wallet._id] || {
              balance: 0,
              totalTransactions: 0,
            ...
            return (
              <div
                ...
                } rounded-xl p-6 border hover:shadow-lg transition-shadow`}
              >
